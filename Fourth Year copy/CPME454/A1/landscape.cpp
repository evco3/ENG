...

{
  ...
  // YOUR CODE HERE
  vec3 seg = segHead - segTail;
  float mul = (position - segTail) * seg / (seg * seg);
  if (mul < 0) {
    return segTail;
  } else if (mul > 1) {
    return segHead;
  } else {
    return segTail + mul * seg;
  }
...
{
  // YOUR CODE HERE
  //find segment that contains x and then calculate distance to that segment
  for (int i=0; i<numVerts-1; i++) {
    if (landscapeVerts[2*i] <= x && landscapeVerts[2*(i+1)] >= x) {
      float y1 = landscapeVerts[2*i+1];
      float y2 = landscapeVerts[2*(i+1)+1];
      float x1 = landscapeVerts[2*i];
      float x2 = landscapeVerts[2*(i+1)];
      return y1 + (y2 - y1) * (x - x1) / (x2 - x1) + 5;
    }
  }
