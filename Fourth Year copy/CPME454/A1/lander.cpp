...

{
  ...
  glBufferData(GL_ARRAY_BUFFER, numPoints * 2 * sizeof(float), landerVerts, GL_STATIC_DRAW);
...
{
  //You will pass MVP to the gpu program.  You can see how it is used in the vertexShader at the top of ll.cpp.
  //To get the vertices, v, of the lander to show up in the correct position, v', in the window, you need to translate and rotate each v according to the current "pose" of the lander ... that is, the lander's position and orientation.  Then you need to move from the world coordindate system to the window coordinate system.
  // YOUR CODE HERE

  mat4 T = translate(position.x, position.y, 0);
  mat4 R = rotate(orientation, vec3(0,0,1));
  mat4 M = T * R ;
...

{
  ...
  if(fuel > 0){
    ...
    this->velocity.x -= THRUST_ACCEL * deltaT * sin(orientation);
    this->velocity.y += THRUST_ACCEL * deltaT * cos(orientation);
