...
#include "world.h"
#include "drawSegs.h"           // segment-drawing for debugging
#include "ll.h"
...
Segs *segs;			// segment-drawing for debugging (JAMES)
...

{
  if (action == GLFW_PRESS)
    ...
    else if (key == 'C')	// c = show closest point
      showClosestPoint = !showClosestPoint;

    else if (key == 'H')	// h = help
      cout << "p - pause (toggle)" << endl
	   << "r - reset" << endl
	   << "c - show closest point (toggle)" << endl;
}
...

{
  ...
  // Set up segment-drawing for debugging
